...
interface BundleLoader {
  (bundle: string, locale: string | null, cb: (error: Error, messages: string[] | IBundledStrings) => void): void;
}
...
async function getMessagesFromTranslationsService(translationServiceUrl: string, language: string, name: string): Promise<string[] | IBundledStrings> {
  const url = endWithSlash(translationServiceUrl) + endWithSlash(language) + 'orbit/' + endWithSlash(name);

  const res = await fetch(url);
  if (res.ok) {
    const message = await res.json() as string[] | IBundledStrings;
    return message;
  }
...
export function load(name: string, req: AMDLoader.IRelativeRequire, load: AMDLoader.IPluginLoadCallback, config: AMDLoader.IConfigurationOptions): void {
  const pluginConfig: INLSPluginConfig = (<any>config)['orbit/nls'] ?? {};
