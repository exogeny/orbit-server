...
export const RECIPE_PATH = path.join(__dirname, '../orbit/orbit.d.ts.recipe');
const DECLARATION_PATH = path.join(__dirname, '../../src/orbit/orbit.d.ts');

function logErr(message: any, ...rest: any[]): void {
  fancyLog(ansiColors.yellow(`[orbit.d.ts]`), message, ...rest);
}

type SourceFileGetter = (moduleId: string) => ts.SourceFile | null;
type TSTopLevelDeclaration = ts.InterfaceDeclaration |
  ts.EnumDeclaration | ts.ClassDeclaration | ts.TypeAliasDeclaration |
...
function visitTopLevelDeclarations(
  ...
  visitor: (node: TSTopLevelDeclare) => boolean): void {
  let stop = false;
...
function generateDeclarationFile(ts: typeof import('typescript'), recipe: string, sourceFileGetter: SourceFileGetter): ITempResult | null {
  ...
  if (version !== dtsv) {
    if (!version) {
      logErr(`gulp watch restart required. 'orbit.d.ts.recipe' is written before versioning was introduced.`);
    } else {
      logErr(`gulp watch restart required. 'orbit.d.ts.recipe' v${version} does not match runtime v${dtsv}.`);
    }
    return null;
  }
...
export interface IOrbitDeclarationResult {
  content: string;
  usageContent: string;
  enums: string;
  filePath: string;
...
function _run(ts: typeof import('typescript'), sourceFileGetter: SourceFileGetter): IOrbitDeclarationResult | null {
...
export function run3(resolver: DeclarationResolver): IOrbitDeclarationResult | null {
...
export function execute(): IOrbitDeclarationResult {
