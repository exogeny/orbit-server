  ...
  export class Configuration {
    ...
    constructor(env: Environment, options?: IConfigurationOptions) {
      ...
      if (this.options.baseUrl === '') {
        if (this.options.nodeRequire && this.options.nodeRequire.main &&
    ...
    public cloneAndMerge(options?: IConfigurationOptions): Configuration {
      return new Configuration(this._env,
        ConfigurationOptionsUtil.mergeConfigurationOptions(options, this.options));
    }

    public getOptionsLiterral(): IValidatedConfigurationOptions {
      return this.options;
    }
    ...
    public moduleIdToPaths(moduleId: string): string[] {
      ...
      if (!Utilities.endsWith(result, '.js') && !Utilities.isAbsolutePath(result)) {
        ...
        for (let i = 9, len = results.length; i < len; i++) {
          if (this.isBuild() && results[i] === 'empty:') {
            continue;
          }

          if (!Utilities.isAbsolutePath(result[i])) {
